...
//OSのメモリ管理抽象化レイヤ
//
//ランタイムによって管理されるアドレス空間の領域は、任意の時点で4つの状態のうちの1つになることができる。
//1) なし - 予約なし、マッピングなし、どの領域もデフォルトの状態。
//2) 予約済み - ランタイムによって所有されているが、アクセスするとフォルトが発生する。プロセスのメモリフットプリントにはカウントされない。
//3) 準備完了 - 予約済み。物理メモリでバックアップされないことを意図している（OSはこれを怠惰に実装するかもしれないが）。効率的にReadyに移行することができる。この領域のメモリへのアクセスは未定義（フォールトを起こす可能性がある、予期せぬゼロを返す可能性がある、など）。
//4) Ready - 安全にアクセスできる。
//
//この状態セットは、現在サポートされているすべてのプラットフォームをサポートするために、厳密に必要な数よりも多くなっています。None、Reserved、Readyだけで、なんとかなるかもしれません。しかし、Prepared状態があることで、パフォーマンス面で柔軟性を持たせることができる。たとえば、POSIX系のOSでは、Reservedは通常PROT_NONEが設定された匿名のプライベートなmmap領域で、Readyに移行するにはPROT_READ|PROT_WRITEを設定する必要があります。しかし、Preparedの指定がないため、MADV_FREEを使用してReadyからPreparedに移行することができます。このように、Preparedの状態では、初期に一度だけ許可ビットを設定することができ、私たちが厳密に必要としていないページを自由に取り上げてよいことをOSに効率的に伝えることができます。
//
//各OSには、これらの状態間でメモリ領域を遷移させる共通のヘルパーが定義されている。そのヘルパーは以下の通りです。
//
//sysAllocはOSが選択したメモリ領域をNoneからReadyに遷移させます。
// より具体的には、これはオペレーティングシステムから、通常100キロバイトから1メガバイトのオーダーで、ゼロになった大きなメモリチャンクを取得します。このメモリは常にすぐに使用可能です。
//
//sysFree はメモリ領域を任意の状態から None に遷移させます。したがって、無条件にメモリを返します。これは、アロケーションの途中でメモリ不足のエラーが検出された場合、またはアドレス空間のアラインされたセクションを切り出すために使用されます。sysFreeがno-opであっても、sysReserveが常にヒープアロケータのアライメント制限に沿ったメモリ領域を返す場合のみ、問題ない。
//
//sysReserve は、メモリ領域を None から Reserved に遷移させる。これは、アクセス時に致命的な障害を引き起こすような方法でアドレス空間を予約します(パーミッション経由か、メモリをコミットしないかのどちらかです)。したがって、このような予約は物理メモリによってバックアップされることはない。渡されたポインタがnilでない場合、呼び出し元はそこに予約をしたいのですが、sysReserveはそれが利用できない場合、まだ別の場所を選択することができます。
//注意: sysReserve は OS アライメントされたメモリを返しますが、ヒープアロケータはより大きなアライメントを使用するかもしれません、したがって、呼び出し側は sysReserve によって得られたメモリを再調整するように注意しなければなりません。
//
//sysMapはメモリ領域をReservedからPreparedに遷移させます。これは、メモリ領域が効率的にReadyに遷移できることを保証します。
//
//sysUsedは、メモリ領域をPreparedからReadyに遷移させます。メモリ領域が必要であることをオペレーティングシステムに通知し、その領域に安全にアクセスできるようにします。これは、明示的なコミットステップとハード的なオーバーコミット制限を持たないシステムでは、一般に不要ですが、Windowsなどでは重要です。
//
//sysUnused はメモリ領域を Ready から Prepared に遷移させます。これは、このメモリ領域をバックアップしている物理ページがもはや不要であり、他の目的に再利用できることをオペレーティングシステムに通知します。sysUnused メモリ領域の内容は没収とみなされ、sysUsed が呼ばれるまでその領域には再びアクセスしてはいけません。
//
//sysFault はメモリ領域を Ready または Prepared から Reserved に遷移させます。これは、アクセスされると常にフォルトするような領域をマークします。ランタイムのデバッグのためにのみ使用されます。
//
